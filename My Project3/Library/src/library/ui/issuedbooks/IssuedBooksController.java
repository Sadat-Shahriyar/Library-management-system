...
public class IssuedBooksController implements Initializable {
    ...
    private void loadData() {
        ...
        try {
            ...
            while (true) {
                ...
                counter += 1;
                String memberName = inFromServer.readLine();
                String bookID = inFromServer.readLine();
                String bookTitle = inFromServer.readLine();
                Timestamp issueTime = Timestamp.valueOf(inFromServer.readLine());
                ...
                Float fine = LibraryUtil.getFineAmount(days);
                IssueInfo issueInfo = new IssueInfo(counter, bookID, bookTitle, memberName, LibraryUtil.formatDateTimeString(new Date(issueTime.getTime())), days, fine);
                list.add(issueInfo);
            }
        } catch (IOException ex) {
            Logger.getLogger(IssuedBooksController.class.getName()).log(Level.SEVERE, null, ex);
    ...
    private void handleReturn(ActionEvent event) {
        IssueInfo issueInfo = (IssueInfo) tableView.getSelectionModel().getSelectedItem();
        if (issueInfo != null) {
